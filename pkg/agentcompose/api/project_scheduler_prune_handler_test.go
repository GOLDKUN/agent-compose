...
func TestProjectHandlerPruneSchedulerRunsMapsRequestAndResult(t *testing.T) {
	...
	runtime.pruneResult = schedulers.SchedulerRunPruneResult{
		DryRun: true,
		Matched: schedulers.SchedulerRunPruneStats{
	...
	if !slices.Equal(runtime.pruneRequest.SchedulerIDs, []string{store.scheduler.ID}) ||
		runtime.pruneRequest.TriggerID != "trigger-history" ||
		!slices.Equal(runtime.pruneRequest.Statuses, []string{domain.SchedulerRunStatusSucceeded, domain.SchedulerRunStatusFailed}) ||
...
func TestProjectHandlerPruneSchedulerRunsRejectsInvalidStatusAndDuration(t *testing.T) {
	...
	if runtime.pruneRequest.SchedulerIDs != nil {
