...
func TestLoaderRequestSandboxConfigHashIgnoresVolumeMountOrder(t *testing.T) {
	...
	first, err := loaderRequestSandboxConfigHash("sha256:loader", domain.SchedulerAgentRequest{}, nil, nil, nil, nil, "docker", "guest:v1", mounts)
	if err != nil {
		t.Fatalf("loaderRequestSandboxConfigHash returned error: %v", err)
	}
	reordered := []domain.SandboxVolumeMount{mounts[1], mounts[0]}
	second, err := loaderRequestSandboxConfigHash("sha256:loader", domain.SchedulerAgentRequest{}, nil, nil, nil, nil, "docker", "guest:v1", reordered)
	if err != nil {
