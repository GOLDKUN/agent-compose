...
func TestLoaderRunPruneFiltersAndDeletesDirectRunData(t *testing.T) {
	...
	filtered, err := store.ListLoaderRunsForPrune(ctx, schedulers.SchedulerRunPruneFilter{
	...
	if remaining, err := store.ListLoaderRunsForPrune(ctx, schedulers.SchedulerRunPruneFilter{SchedulerIDs: []string{"loader-a", "loader-b"}}); err != nil || len(remaining) != 4 {
...
func TestDeleteLoaderRunPruneDataRollsBackWholeTransaction(t *testing.T) {
	...
	for _, runID := range []string{"run-ok", "run-fail"} {
		if err := store.CreateLoaderRun(ctx, domain.SchedulerRunSummary{ID: runID, SchedulerID: "loader-a", TriggerID: "trigger-a", Status: domain.SchedulerRunStatusSucceeded, StartedAt: now, CompletedAt: now}); err != nil {
...
func TestDeleteLoaderRunPruneDataRechecksTerminalTriggerRun(t *testing.T) {
	...
	if counted, err := store.CountLoaderRunPruneData(ctx, keys); err != nil || counted != (schedulers.SchedulerRunPruneDatabaseStats{}) {
		t.Fatalf("counted=%#v err=%v", counted, err)
	}
	if removed, err := store.DeleteLoaderRunPruneData(ctx, keys); err != nil || removed.Stats != (schedulers.SchedulerRunPruneDatabaseStats{}) || len(removed.RemovedKeys) != 0 {
...
func createPruneTestLoader(t *testing.T, store *ConfigStore, loaderID string) {
	...
	if _, err := upsertNativeTestScheduler(context.Background(), store, domain.Scheduler{
		Summary: domain.SchedulerSummary{
			ID: loaderID, Name: loaderID, Runtime: domain.SchedulerRuntimeScheduler,
...
func addPruneTestRelations(t *testing.T, store *ConfigStore, loaderID, runID, triggerID string) {
	t.Helper()
	ctx := context.Background()
	if err := store.AddLoaderEvent(ctx, domain.SchedulerEvent{ID: "loader-event-" + runID, SchedulerID: loaderID, RunID: runID, TriggerID: triggerID, Type: "loader.run.completed", CreatedAt: time.Now().UTC()}); err != nil {
		t.Fatalf("add loader event: %v", err)
	}
	if _, err := store.db.ExecContext(ctx, `INSERT INTO event_delivery(event_id, scheduler_id, trigger_id, scheduler_run_id, status, created_at, updated_at) VALUES(?, ?, ?, ?, 'run_succeeded', 1, 1)`, "event-"+runID, loaderID, triggerID, runID); err != nil {
		t.Fatalf("add event delivery: %v", err)
	}
	if _, err := store.db.ExecContext(ctx, `INSERT INTO event_sandbox_link(event_id, sandbox_id, relation, scheduler_id, scheduler_run_id, trigger_id, created_at) VALUES(?, ?, 'used', ?, ?, ?, 1)`, "event-"+runID, "sandbox-"+runID, loaderID, runID, triggerID); err != nil {
