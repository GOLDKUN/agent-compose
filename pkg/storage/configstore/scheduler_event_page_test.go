...
func TestLoaderEventPageOnlyReturnsEventsJoinedToTriggerRuns(t *testing.T) {
	...
	first, err := store.ListLoaderEventsPage(ctx, schedulers.SchedulerEventPageFilter{SchedulerIDs: []string{"loader-a"}, RequireTrigger: true, TriggerID: "trigger-a", Limit: 1})
	if err != nil || len(first) != 1 || first[0].ID != "event-a2" || first[0].TriggerID != "trigger-a" {
		t.Fatalf("first page=%#v err=%v", first, err)
	}
	second, err := store.ListLoaderEventsPage(ctx, schedulers.SchedulerEventPageFilter{
		SchedulerIDs: []string{"loader-a"}, RequireTrigger: true, TriggerID: "trigger-a", BeforeCreatedAt: first[0].CreatedAt,
		BeforeLoaderID: first[0].SchedulerID, BeforeEventID: first[0].ID, Limit: 10,
	})
	if err != nil || len(second) != 1 || second[0].ID != "event-a1" {
		t.Fatalf("second page=%#v err=%v", second, err)
	}
	byRun, err := store.ListLoaderEventsPage(ctx, schedulers.SchedulerEventPageFilter{SchedulerIDs: []string{"loader-a"}, RequireTrigger: true, RunID: "run-b", Limit: 10})
...
func TestLoaderEventPageSupportsTailBoundaryAndAscendingRange(t *testing.T) {
	...
	if err := store.CreateLoaderRun(ctx, domain.SchedulerRunSummary{ID: "run-a", SchedulerID: "loader-a", TriggerID: "trigger-a", Status: domain.SchedulerRunStatusSucceeded, StartedAt: startedAt}); err != nil {
		t.Fatalf("create run: %v", err)
	}
	for index, id := range []string{"event-1", "event-2", "event-3", "event-4"} {
		if err := store.AddLoaderEvent(ctx, domain.SchedulerEvent{SchedulerID: "loader-a", ID: id, RunID: "run-a", TriggerID: "trigger-a", Type: "loader.log", CreatedAt: startedAt.Add(time.Duration(index) * time.Second)}); err != nil {
			t.Fatalf("add event %s: %v", id, err)
		}
	}
	boundary, err := store.ListLoaderEventsPage(ctx, schedulers.SchedulerEventPageFilter{SchedulerIDs: []string{"loader-a"}, RequireTrigger: true, Limit: 1, Offset: 1})
	if err != nil || len(boundary) != 1 || boundary[0].ID != "event-3" {
		t.Fatalf("tail boundary = %#v, err=%v", boundary, err)
	}
	page, err := store.ListLoaderEventsPage(ctx, schedulers.SchedulerEventPageFilter{
		SchedulerIDs: []string{"loader-a"}, RequireTrigger: true, Ascending: true, Limit: 10,
		AfterCreatedAt: boundary[0].CreatedAt, AfterLoaderID: boundary[0].SchedulerID, AfterEventID: boundary[0].ID,
