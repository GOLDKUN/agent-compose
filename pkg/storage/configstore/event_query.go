...
func scanEventSummary(scan func(dest ...any) error) (domain.EventSummary, error) {
	...
	item.DispatchedAt = ParseStoredTime(dispatchedAt)
	return item, nil
}
...
func (s *eventStore) ListEventTopics(ctx context.Context, source string, offset, limit int) ([]domain.EventTopicSummary, int, error) {
	...
	for rows.Next() {
		...
		item.LatestEventAt = ParseStoredTime(latestAt)
		items = append(items, item)
	}
	if err := rows.Err(); err != nil {
...
func (s *eventStore) listEventRunTraces(ctx context.Context, eventIDs []string) ([]domain.EventRunTrace, error) {
	...
	for rows.Next() {
		...
		item.Delivery.CreatedAt = ParseStoredTime(deliveryCreatedAt)
		item.Delivery.UpdatedAt = ParseStoredTime(deliveryUpdatedAt)
		if schedulerID != "" {
		...
		if run.ID != "" {
			run.StartedAt = ParseStoredTime(runStartedAt)
			run.CompletedAt = ParseStoredTime(runCompletedAt)
			item.Run = &run
