...
func TestIntegrationImageCacheLocalWorkflows(t *testing.T) {
	...
	t.Run("lock cancellation", TestLockContextStopsWaitingWhenCanceled)
	t.Run("error kind", TestErrorKindSupportsErrorsIsAndAs)
	t.Run("parse reference", TestParseReferenceUsesGoContainerRegistry)
	t.Run("cache helper workflows", TestImageCacheHelperCoverageWorkflows)
	t.Run("copy directory hardlink", TestImageCacheCopyDirHardlinkFirstCoverage)
	...
	t.Run("remove prune children", TestRemovePruneChildrenDoesNotDeleteMaterializedCache)
	t.Run("prune protected image", TestPruneBeforeUsesLastUsedAndProtectsReferencedImage)
	t.Run("prune image usage time", TestImageUsedAtPrefersLastUsedAndFallsBackToPulled)
	t.Run("prune old materialization", TestPruneBeforeRemovesOldUntrackedMaterialization)
	t.Run("prune retained alias", TestPruneBeforePreservesMaterializationUsedByRetainedAlias)
	t.Run("prune mutable sandbox reference", TestPruneBeforePreservesUntrackedMaterializationForMutableSandboxRef)
	...
	t.Run("normalize wrapper and invalid ref", TestNormalizeReferenceWrapperAndInvalidInput)
