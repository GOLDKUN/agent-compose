...
func TestIntegrationLifecycleRecoversInterruptedStoppedRuntimeRelease(t *testing.T) {
	...
	lifecycle := sessions.Lifecycle{Config: config, Store: store, Driver: driver, Locks: sessions.NewLifecycleLocks()}
...
func TestLifecycleSerializesRuntimeReleaseAgainstResume(t *testing.T) {
	...
	lifecycle := sessions.Lifecycle{Config: config, Store: store, Driver: driver, WorkspaceEnsurer: releaseWorkspaceEnsurer{}, Locks: locks}
...
func TestLifecycleSerializesRuntimeReleaseAgainstRemoval(t *testing.T) {
	...
	locks := sessions.NewLifecycleLocks()
	lifecycle := sessions.Lifecycle{Config: config, Store: store, Driver: driver, Locks: locks}
	removal := &sessions.RemovalCoordinator{SandboxRoot: config.SandboxRoot, Store: store, Runtime: driver, Locks: locks}
...
func newReleaseRecoverySandbox(t *testing.T) (*appconfig.Config, *sessionstore.Store, *domain.Sandbox) {
	t.Helper()
	root := t.TempDir()
	...
	sandbox, err := store.CreateSandboxWithOptions(context.Background(), "release", "", "docker", "guest:latest", "", "test", nil, nil, nil, sessionstore.CreateSandboxOptions{
