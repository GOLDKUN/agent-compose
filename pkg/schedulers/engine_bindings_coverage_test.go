...
type coverageEngineHost struct {
	...
	agentCalls   []domain.SchedulerAgentRequest
	llmCalls     []domain.SchedulerLLMRequest
	commandCalls []domain.SchedulerCommandRequest
...
func (h *coverageEngineHost) Agent(_ context.Context, _ string, request domain.SchedulerAgentRequest) (domain.SchedulerAgentResult, error) {
...
func (h *coverageEngineHost) LLM(_ context.Context, _ string, request domain.SchedulerLLMRequest) (domain.SchedulerLLMResult, error) {
	h.llmCalls = append(h.llmCalls, request)
	text := "llm-output"
	if strings.TrimSpace(request.OutputSchema) != "" {
		text = `{"summary":"ok","risk":"low"}`
	}
	return domain.SchedulerLLMResult{Text: text, Model: firstNonEmptyTest(request.Model, "gpt"), ResponseID: "resp-1", FinishReason: "stop"}, nil
}

func (h *coverageEngineHost) Command(_ context.Context, request domain.SchedulerCommandRequest) (domain.SchedulerCommandResult, error) {
	h.commandCalls = append(h.commandCalls, request)
	return domain.SchedulerCommandResult{Stdout: "command-output", Output: "command-output", ExitCode: 0, Success: true, SandboxID: "command-session", CellID: "command-cell", Artifacts: map[string]string{"stdout": "/tmp/stdout.txt"}}, nil
}
...
func (h *coverageEngineHost) CallSandboxRPC(_ context.Context, method, requestJSON string) (string, error) {
	if h.requests == nil {
	...
	const sandboxID = "sandbox-from-host"
	switch method {
	case "CreateSandbox", "GetSandbox", "ResumeSandbox":
		return `{"sandbox":{"summary":{"sandboxId":"` + sandboxID + `","vmStatus":"RUNNING"}}}`, nil
	case "ListSandboxes":
...
func TestQJSSchedulerEngineBindingCoverageWorkflow(t *testing.T) {
	...
	result, err := engine.Execute(context.Background(), SchedulerExecutionRequest{
...
function main(payload) {
  ...
  const agent = scheduler.agent("summarize", {
    agent: "claude", sandboxPolicy: "new", timeout: "45s", title: "Scheduler Agent Session",
  ...
  return { current, sandboxes, proxy, stopped, resumed, agent, llm, execResult, shellResult, existing, published, runtime: scheduler.runtime.name };
	...
	if host.agentCalls[0].SandboxPolicy != domain.SchedulerSandboxPolicyNew || domain.SandboxEnvMap(host.agentCalls[0].SandboxEnv)["REQUEST_ONLY"] != "request" {
		t.Fatalf("scheduler.agent sandbox options mapped to %#v", host.agentCalls[0])
	}
	if host.commandCalls[0].SandboxPolicy != domain.SchedulerSandboxPolicyNew {
		t.Fatalf("scheduler.exec sandboxPolicy mapped to %#v", host.commandCalls[0])
	}
	if got := host.requests["GetSandbox"]["sandboxId"]; got != "sandbox-from-host" {
		t.Fatalf("scheduler.sandbox request = %#v", host.requests["GetSandbox"])
	}
	...
	if len(result.Warnings) != 0 || !strings.Contains(result.ResultJSON, `"runtime":"scheduler"`) || !strings.Contains(result.ResultJSON, `"eventId":"evt-test"`) || !strings.Contains(result.ResultJSON, `"sandboxId":"sandbox-from-host"`) {
		t.Fatalf("result json = %s", result.ResultJSON)
	}

	triggerResult, err := engine.Execute(context.Background(), SchedulerExecutionRequest{
		Runtime:     domain.SchedulerRuntimeScheduler,
		PayloadJSON: `{"input":true}`,
		Trigger:     &domain.SchedulerTrigger{ID: "cron-id"},
	...
	singleTriggerResult, err := engine.Execute(context.Background(), SchedulerExecutionRequest{
		Runtime:     domain.SchedulerRuntimeScheduler,
		PayloadJSON: `{"value":3}`,
	...
	optionsHost := &coverageEngineHost{}
	optionsResult, err := engine.Execute(context.Background(), SchedulerExecutionRequest{
		Runtime: domain.SchedulerRuntimeScheduler,
		Script: `
function main() {
  scheduler.agent("snake options", { sandbox_policy: "new", sandbox_env: { FROM_SNAKE: "yes" } });
  scheduler.exec({ command: "true", sandboxPolicy: "reuse", sandboxEnv: [{ name: "FROM_CAMEL", value: "yes" }] });
}`,
	}, optionsHost)
	if err != nil {
		t.Fatalf("sandbox options execute returned error: %v", err)
	}
	if len(optionsHost.agentCalls) != 1 || optionsHost.agentCalls[0].SandboxPolicy != domain.SchedulerSandboxPolicyNew || domain.SandboxEnvMap(optionsHost.agentCalls[0].SandboxEnv)["FROM_SNAKE"] != "yes" {
		t.Fatalf("scheduler.agent sandbox option mapping = %#v", optionsHost.agentCalls)
	}
	if len(optionsHost.commandCalls) != 1 || optionsHost.commandCalls[0].SandboxPolicy != domain.SchedulerSandboxPolicySticky || domain.SandboxEnvMap(optionsHost.commandCalls[0].SandboxEnv)["FROM_CAMEL"] != "yes" {
		t.Fatalf("scheduler.exec sandbox option mapping = %#v", optionsHost.commandCalls)
	}
	if len(optionsResult.Warnings) != 0 {
		t.Fatalf("unexpected sandbox option warnings: %#v", optionsResult.Warnings)
	}

	validation, err := engine.Validate(context.Background(), domain.SchedulerRuntimeScheduler, `
...
`)
	...
	warningOnly, err := engine.Validate(context.Background(), domain.SchedulerRuntimeScheduler, `const value = 1;`)
	if err != nil || len(warningOnly.Warnings) != 1 {
		t.Fatalf("warning validation=%#v err=%v", warningOnly, err)
	}
}

func TestIntegrationQJSSchedulerEngineBindingCoverageWorkflow(t *testing.T) {
	TestQJSSchedulerEngineBindingCoverageWorkflow(t)
}

func TestE2EQJSSchedulerEngineBindingCoverageWorkflow(t *testing.T) {
	TestQJSSchedulerEngineBindingCoverageWorkflow(t)
}
...
func TestQJSSchedulerEngineValidationCoverageWorkflow(t *testing.T) {
	...
	tests := []struct {
		...
		{`scheduler.session.getSession({ sessionId: "session-1" })`, "cannot read property 'getSession' of undefined"},
	...
	for _, tt := range execTests {
		...
		if strings.Contains(tt.wantErr, "trigger missing") {
			request.Trigger = &domain.SchedulerTrigger{ID: "missing"}
		}
...
func TestIntegrationQJSSchedulerEngineValidationCoverageWorkflow(t *testing.T) {
	TestQJSSchedulerEngineValidationCoverageWorkflow(t)
}

func TestE2EQJSSchedulerEngineValidationCoverageWorkflow(t *testing.T) {
	TestQJSSchedulerEngineValidationCoverageWorkflow(t)
}
