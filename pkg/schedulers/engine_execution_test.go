...
func TestQJSSchedulerEnginePreservesCallbackObjectResult(t *testing.T) {
...
func TestQJSSchedulerEngineUsesCapturedJSONStringifier(t *testing.T) {
	result, err := (&QJSSchedulerEngine{}).Execute(context.Background(), SchedulerExecutionRequest{
		Runtime: domain.SchedulerRuntimeScheduler,
		Script: `
...
func TestQJSSchedulerEngineReportsUnserializableResult(t *testing.T) {
...
func TestQJSSchedulerEngineAcceptsNullLogPayload(t *testing.T) {
...
func TestQJSSchedulerEngineCancellationInterruptsPendingPromise(t *testing.T) {
...
scheduler.interval("pending", async function pending() {
  ...
  await new Promise(function neverResolve() {});
			...
			Trigger: &domain.SchedulerTrigger{ID: "pending"},
		}, host)
		result <- err
	}()
...
func TestQJSSchedulerEnginePreservesSuccessfulResultWhenContextIsCanceledAfterExecution(t *testing.T) {
	ctx, cancel := context.WithCancelCause(context.Background())
	stopCause := errors.New("late stop request")
	want := SchedulerExecutionResult{ResultJSON: `{"ok":true}`, Warnings: []string{"preserved"}}

	result, err := executeWithQJSPanicRecovery(ctx, func() (SchedulerExecutionResult, error) {
		cancel(stopCause)
		return want, nil
	})
	if err != nil {
