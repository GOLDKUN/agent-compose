...
agent, scheduler, and run ids are generated by stable rules.
...
- Write native `project_agent` and `project_scheduler` records.
- Derive agent and scheduler runtime definitions from the selected immutable
  `project_revision.spec_json`; definitions are not persisted as projection
  tables.
- Compile declarative `scheduler.triggers` into the scheduler script and write
  validated trigger state to `scheduler_trigger`. Inline `scheduler.script` is
  used directly and its validated triggers update
  `ProjectScheduler.trigger_count`.
- Disable schedulers removed from the spec, then refresh the scheduler
  controller.
...
Project reconciliation modifies only records owned by that project and revision.
Runtime definitions remain projections in memory from immutable revision data.
...
- Project runs: the runs controller passes `run.AgentID` into
  `ExecuteAgentRequest`
- Scheduler runs: `RuntimeHost.Agent` passes the scheduler-bound agent id
- Existing sandboxes can recover agent identity from the `agent_id` tag
...
Agent and scheduler ownership is represented directly by `project_id`,
`revision`, `agent_name`, and `scheduler_id` on the native project tables.
