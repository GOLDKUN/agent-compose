...
- 包含 `/`：分割为 server name 和传给 OctoBus 的真实 capset ID；
- server name 为空或 capset ID 为空是配置错误；
- server name 使用现有 stable identifier 校验，因此不能包含 `/`；
- 真实 capset ID 必须符合 OctoBus 的 `^[a-zA-Z][a-zA-Z0-9_-]{0,62}$`
  规则，因此不能包含 `/`；一个声明最多只能包含一个 `/`；
...
- 缺失 server 引用；
- 多个 `/`、不符合 OctoBus ID 规则的 capset 拒绝；
...
- 有一个 `/` 的 capset 使用前半部分选择 project server，后半部分必须是合法的
  OctoBus capset ID；
